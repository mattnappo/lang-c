// For `SpanRemover` traversal
use ast_nospan;
use visit;
use visit::Visit;

fn remove_span(unit: &TranslationUnit) -> ast_nospan::TranslationUnit {
    let mut remover = SpanRemover(ast_nospan::TranslationUnit(vec![]));

    visit::visit_translation_unit(&mut remover, &unit);

    todo!()
}

/// Remove `Span`/`Node`s from an AST (clean it)
#[derive(Debug)]
struct SpanRemover(ast_nospan::TranslationUnit);

use ast::*;
use span::Span;

impl<'ast> visit::Visit<'ast> for SpanRemover {
